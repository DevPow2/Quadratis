...
Snake::Snake(Communication* comm, Display_Master *display)
{
    ...
    displays->drawRect(snake.beenHeadOnDisplay[counter], snake.headX, snake.headY, snake.width, snake.height, ILI9341_BLACK);
    ...
    displays->setTextColor(ILI9341_BLACK); //Score keeper
...
void Snake::clearObject(int display, int x, int y)
{
    for (int i = y; i < y + snake.height; i++)
    {
        ...
        {
            displays->clearPixel(display, j, i, ILI9341_CYAN);
        }
    }
}
...
void Snake::run() 
{
    ...
    if(snake.headX > 320 || snake.headY > 240)
    {
        currentDisplay =  displays->getNextScreen();
        if(snake.headX > 320) snake.headX = 0;
        if(snake.headY > 240) snake.headY = 0;
        
    }
    if(snake.beenHeadOnDisplay[counter] == 2 && snake.beenHeadOnDisplay[counter] == 3) 
        {
            snake.changeX = 0;
            snake.changeY = -10;
        }
        else if(snake.beenHeadOnDisplay[counter] == 3 && snake.headY > 100)
        {
            snake.changeX = 10;
            snake.changeY = 0;
        }
        else if(snake.beenHeadOnDisplay[counter] == 3 && snake.headX > 150)
        {
            snake.changeX = 0;
            snake.changeY = 10;
        }   
        else if(snake.beenHeadOnDisplay[counter] == 2 && snake.headY > 100)
        {
            snake.changeX = 0;
            snake.changeY = 10;
        }
...
void Snake::printScore()
{
    ...
    displays->fillRectAllDisplays(130, 3, 50, 16, ILI9341_CYAN); //clears old score
